...
- **Despliegue:** repo importado en Vercel → **https://cabanaslamaite.vercel.app** (auto-deploy en push a main). `api/` se detecta solo. UID/descripción del feed .ics usan el host desde el que se consulta (adaptativo). ⚠️ El dueño compró `cabañaslamaite.com` (xn--cabaaslamaite-lkb.com) y pidió **reembolso** (el formato punycode al compartir no le gustó); volvió a usar el subdominio `.vercel.app`. Plan: comprar `cabanaslamaite.com` (sin ñ, libre) cuando se procese el reembolso — los feeds iCal y el código se adaptan solos al host.
